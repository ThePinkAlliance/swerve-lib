package com.ThePinkAlliance.swervelib.rev;
...
public final class NeoSteerControllerFactoryBuilder {
  ...
  public NeoSteerControllerFactoryBuilder withVoltageCompensation(
  ...
  public NeoSteerControllerFactoryBuilder withCurrentLimit(
      double currentLimit) {
  ...
  public class FactoryImplementation<T>
      implements
      SteerControllerFactory<ControllerImplementation, NeoSteerConfiguration<T>> {
    ...
    public ControllerImplementation create(
        NeoSteerConfiguration<T> steerConfiguration,
        ModuleConfiguration moduleConfiguration) {
      AbsoluteEncoder absoluteEncoder = encoderFactory.create(
          steerConfiguration.getEncoderConfiguration());

      CANSparkMax motor = new CANSparkMax(
          steerConfiguration.getMotorPort(),
  ...
  public static class ControllerImplementation implements SteerController {
    ...
    private static final double ENCODER_RESET_MAX_ANGULAR_VELOCITY = Math.toRadians(
        0.5);
    ...
    @Override
    public void configureStatusFrame(StatusFrame frame, int period) {
      this.motor.setControlFramePeriodMs(period);
    }
