...
import com.ThePinkAlliance.swervelib.rev.NeoSteerControllerFactoryBuilder;
...
public final class Mk4SwerveModuleHelper {
  ...
  private static DriveControllerFactory<?, Integer> getFalcon500DriveFactorySim(
      Mk4ModuleConfiguration configuration, CtrePhysicsSim sim) {
    return new Falcon500DriveControllerFactoryBuilder()
        .withVoltageCompensation(configuration.getNominalVoltage())
        .withCurrentLimit(configuration.getDriveCurrentLimit())
        .withSimulation(sim)
  ...
  public static SwerveModule createFalcon500Sim(
      ...
      int steerMotorPort,
      int steerEncoderPort,
      double steerOffset, CtrePhysicsSim sim) {
    return new SwerveModuleFactory<>(
        gearRatio.getConfiguration(),
        getFalcon500DriveFactorySim(configuration, sim),
        getFalcon500SteerFactory(configuration))
        .create(
            container,
            driveMotorPort,
   ...
   * @param steerEncoderPort The CAN ID of the steer CANCoder.
   * @param steerOffset      The offset of the CANCoder in radians.
   * @param sim              The ctre physics simulator.
   * @return The configured swerve module.
   */
  public static SwerveModule createFalcon500(
      ShuffleboardLayout container,
